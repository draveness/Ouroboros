...
- (id)calculateInternalValueWithPercent:(CGFloat)percent {
    ...
    if ([self.fromValue isKindOfClass:[NSNumber class]] && [self.toValue isKindOfClass:[NSNumber class]]) {
        ...
        NSString *toValueType = [NSString stringWithCString:[self.toValue objCType] encoding:NSStringEncodingConversionAllowLossy];
        if ([valueType isEqualToString:toValueType]) {
            ...
            NSAssert(NO, @"fromValue type is different from toValue type.");
        }
    } else {
        NSAssert(NO, @"fromValue type is different from toValue type.");
    }
